export class CryptoUser {
    ...
    encryptedTestResults: string[] = [];

    private abi: any = null;
    private contractAddress: string = "0x438cFd691017711468fcE90c57907A7d637A5033";
    ...
    async writeTestResult(hash: string | null, amount: string | null): Promise<void> {
        if (!this.address) {
    ...
    async readTestResults(address?: string): Promise<void> {
        try {
    ...
    async checkContractBalance(): Promise<void> {
        const provider = new ethers.BrowserProvider((window as any).ethereum);
